...
int main(int argc, char **argv)
{
	...
	for (auto it = de.begin(); it != de.end(); it++) {
		//doit(*it, 18);
		doit(*it, 10);
		for (auto it2 = mp[{*it, 10}].mpi.begin(); it2 != mp[{*it, 10}].mpi.end(); it2++) {
			if (mp.find({(*it2).first, 7}) == mp.end()) {
				doit((*it2).first, 7);
				for (auto it3 = mp[{(*it2).first, 7}].mpi.begin(); it3 != mp[{(*it2).first, 7}].mpi.end(); it3++) {
					if (mp.find({(*it3).first, 8}) == mp.end()) {
						doit((*it3).first, 8);
					}
				}
			}
	...
	for (auto it = de.begin(); it != de.end(); it++) {
		for (auto it2 = mp[{*it, 10}].mpi.begin(); it2 != mp[{*it, 10}].mpi.end(); it2++) {
			...
			for (auto it3 = mp[{(*it2).first, 7}].mpi.begin(); it3 != mp[{(*it2).first, 7}].mpi.end(); it3++) {
				...
				for (auto it4 = mp[{(*it3).first, 8}].mpi.begin(); it4 != mp[{(*it3).first, 8}].mpi.end(); it4++) {
