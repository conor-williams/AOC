...
int main(int argc, char **argv)
{
	...
	int var_max = 0;
	do {
		if (leny != 2) {printf("exit 0 \n"); exit(0);}
		///ORIG STUFF
		int var_one = 0; int var_two = 0;
...
again:
		...
		for (int i = 0; i < (int)line2.length(); i++) {
			...
			if (isalfa(line2[i])) {
				...
				cout << "**c_ns: " << getPlaceHolder(line2[i]) << endl;
				if (getPlaceHolder(line2[i]) > var_max) {var_max = getPlaceHolder(line2[i]);}
			}
		}
	} while (next_permutation(combo, combo+lenyOrig));
