...
int main(int argc, char **argv)
{
        ...
        a = fopen(argv[1], "r"); printf("2021 day17 part2\n"); fflush(stdout);
...
fclose(a);
	...
	for (int j = 0; j < 4300; j++) {
		...
		vals[j] = new1;
		//printf("j:%d -- %d - (%d)\n", j, new1, new1-perv);
		//printf("%d", new1-perv); fflush(stdout);
		//printf("%d\n", j);
		diffs[j] = new1-perv;
		perv = new1;
	...
	for (int ii = 0; ii < 6400; ii++) {
		...
		if (found == 0) {
			printf("start of cycle is ii:%d\n", ii);
			startCy = ii;
			break;
		}
	}

	int posTrill = (unsigned long long) (1000000000000 - startCy ) % mincyc;
	printf("posTrill is %d\n", posTrill);
	printf("posTrill+startCy is %d\n", posTrill+startCy);
	int cena = 0;
	...
	printf("sumDiffs: %d\n", sumDiffs);
	assert(vals[startCy]+sumDiffs == vals[startCy+mincyc]);
	assert(vals[startCy]+(2*sumDiffs) == vals[startCy+(2*mincyc)]);
	/*
	assert(vals[startCy]+sumDiffs == vals[startCy]+(*mincyc)]);
	assert(vals[startCy]+sumDiffs == vals[startCy]+(*mincyc)]);
	*/
	...
	unsigned long long numCycles = (unsigned long long) ((unsigned long long)1000000000000 - (unsigned long long)startCy ) / (unsigned long long) mincyc;
	printf("numCycles %llu\n", numCycles);
	printf("%d diffs[startCy] %d diffs[startCy+1] %d\n", diffs[(2*startCy) - 1], diffs[startCy], diffs[startCy+1]);
	printf("**ans: %llu\n", (unsigned long long)(vals[startCy]+(numCycles*sumDiffs)+cena));
/*
	for (int ii = 1000; ii < 6000; ii++) {
	}
	int cy1 = findCycle();
	*/
