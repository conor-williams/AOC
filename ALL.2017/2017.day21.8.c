...
*/
			...
			if (numSq == 1) {
				...
				printf("NOT USED	to test sq 4 of 3s\n");
...
void genallTwos() {
	...
	for (int i = 0; i < twosPos; i++) {
		...
		for (int k = 1; k < 4; k++) { //rotate
			if (k == 1) {
				new[newPos].frm[(1+0)%4] = twos[i].frm[0];
				new[newPos].frm[(1+2)%4] = twos[i].frm[1];
				new[newPos].frm[(1+3)%4] = twos[i].frm[2];
				new[newPos].frm[(1+1)%4] = twos[i].frm[3];
				new[newPos].frm[4] = '\0';
				strcpy(new[newPos].to, twos[i].to);
				newPos++;
			} else {
				new[newPos].frm[(1+0)%4] = new[newPos-1].frm[0];
				new[newPos].frm[(1+2)%4] = new[newPos-1].frm[1];
				new[newPos].frm[(1+3)%4] = new[newPos-1].frm[2];
				new[newPos].frm[(1+1)%4] = new[newPos-1].frm[3];
				new[newPos].frm[4] = '\0';
				strcpy(new[newPos].to, twos[i].to);
				newPos++;
			}
