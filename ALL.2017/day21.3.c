	...
	for (int t = 0; t < 5; t++) {
		...
		if (strlen(start) % 2 == 0) {
			...
			if (numSq == 1) {
				printf("NOT USED\n");
				char p2[10] = "";
				p2[0] = start[0]; p2[1] = start[1]; p2[2] = start[2];
				p2[3] = start[3]; p2[4] = '\0';
				convTwo(p2, 0);
			} else if (numSq == 4) {
...
void genRealAns(int num, int size) {
	...
	if (num == 2) {
		...
		for (int i = 0; i < (int)strlen(ans[j]); i++) { if (ans[j][i] == '#') { mytot++; } }		
			...
			printf("in getReal my36-2tot is %d\n", mytot);
		}
			for (int i = 0; i < 6; i++) {
		...
		for (int i = 0; i < (int)strlen(ans[j]); i++) { if (ans[j][i] == '#') { mytot++; } }		
			...
			printf("in getReal my9-2tot is %d\n", mytot);
		}
			for (int i = 0; i < 3; i++) {
...
*/
		...
		for (int i = 0; i < (int)strlen(ans[j]); i++) { if (ans[j][i] == '#') { mytot++; } }		
			...
			printf("in getReal my4-2tot is %d\n", mytot);
		}
			for (int i = 0; i < 2; i++) {
		...
		for (int i = 0; i < (int)strlen(ans[j]); i++) { if (ans[j][i] == '#') { mytot++; } }		
			...
			printf("in getReal my9-3tot is %d\n", mytot);
		}
			for (int i = 0; i < 3; i++) {
