...
while (1) 
{
	...
	for (int i = 1; i <= max; i++) {
		...
		if (found == 0) {
			sort(already, already+alreadyPos);
			int found22 = 0;
			for (int k = 0; k < groupsPos; k++) {
				for (int i = 0; i < alreadyPos; i++) {
					if (already[i] == groups[k][i]) {
						found22++;
					}
				}
			}
			if (found22 == alreadyPos) {} else {
				for (int i = 0; i < alreadyPos; i++) {
					groups[groupsPos][i] = already[i];
				}
				groupsPos++;
			}
			errors[errorsPos++] = i;
	...
	printf("notfail (plus the 0 (not inc)) is %d\n", notfail);
