	...
	for (int GE = 1; GE <= 2; GE++) {
		...
		if (point[po].GE == GE) { //TOP
			for (int i = 0; i < pointPos; i++) {
				...
				if (grid[point[po].y][point[po].x-1] != '#') {
					for (int y = 0; y<100; y++) { for (int x = 0; x < 100; x++) { already[y][x] = -1; } }
					already[point[po].y][point[po].x-1] = 0;
					already[point[po].y][point[po].x] = 0;
					minPath = 9999;
					getMinDist(point[po].x-1, point[po].y, 0, po, i, point[i].x, point[i].y);
					man[1] = minPath;
...
again2:
			...
			///if (moved == 1 && point[po].remove != 1 && point[po].shotAL == 0) { point[po].hitpoints -= 3; }
		}
