...
//		printf("x4 is %d\n", x4);
//	for (volatile int y4 = 0; y4 <= leny; y4++) {
//	for (volatile int z4 = 0; z4 <= leny; z4++) {
	int x4= 0; int y4 = 0; int z4 = 0;
	for (; x4 <= leny; x4++, y4++, z4++) {
	{
	{
		int semitot = 0;
		for (int i = 0; i < leny; i++) {
			if ((abs(xyzr[i].x1 - xyzr[x4].x1) + abs(xyzr[i].y1 - xyzr[y4].y1) + abs(xyzr[i].z1 - xyzr[z4].z1)) <= abs(xyzr[i].r1)) {
				semitot++;
			}
		}
