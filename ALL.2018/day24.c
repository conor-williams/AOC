...
struct group_s immune[20];
int immunePos = 0;
struct group_s both[40];
int bothPos = 0;
struct group_s infection[20];
int infectionPos = 0;
int conv(char type[]);
...
again44:
	...
	for (int i = 0; i < bothPos; i++) {
		if (both[i].type == 1) {
			printf("IMM: %d (ini:%d) (power:%d)\n", both[i].units, both[i].ini,
				both[i].units*both[i].damage);
			immCount++;
			tot+= both[i].units;
		} else if (both[i].type == 2) {
...
///////MAIN LOOP 
	...
	for (int i = 0; i < bothPos; i++) {
		...
		if (attd.atk == -1) {
			//printf("ATTR SEL is %d ATTD is %d\n", both[attr].units, both[attd.itsI].units);
			printf("SETTING ignore for ^^^^^^ %d\n", attr); both[attr].ignore = 1; continue;
		} else {
