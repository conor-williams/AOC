...
int main(int argc, char **argv)
{
	...
	printf("	must compile with gcc (auto function + pointers)\n");
	printf("	SLOW ~2mins\n");
...
again3:
	...
	for (int i = 0; i < pointPos; i++) {
		...
		if (point[i].GE == 1) {
			...
			printf("\x1b[%dC \x1b[%dB [G: %3d]", 37+(num*9), point[i].y, point[i].hitpoints);
			prevY = point[i].y;
		} else {
			if (prevY == point[i].y) {
				num++;
			...
			printf("\x1b[%dC \x1b[%dB [E: %3d]", 37+(num*9), point[i].y, point[i].hitpoints);
			prevY = point[i].y;
		}
	}
	printf("\x1b[H");
...
again5:
		...
		//if (ntmpdists[0].dist == 0) {goto attack;}
...
if (move == 1 && ntmpdists[0].dist != 0) {
		...
		if (qdist == 0) {/*printf("should go in\n");*/}
		if ((qdist == 0) && moved == 1 && once == 1 && mypos > 0) {
			once++;
			//printf("---- just moved... po:%d moved is %d \n", po, moved); 
			myPP = 0;
			//printf("mypos before calcdist...%d goto restartNEXT\n", mypos);
			move = 0;
			moved = 0;
			goto restartNext;
			mypos = myPP;
...
again2:
		...
		if (pos > 3 && tmpdists[0].hits == tmpdists[1].hits && tmpdists[0].hits == tmpdists[2].hits 
			...
			if (point[tmpdists[0].itsi].remove != 1 && point[po].remove != 1) {
				//printf("minus here1 (only 1) 3 tmpdists[0].itsi:%d po: %d\n", tmpdists[0].itsi, po);
				point[po].shotAL = 1;
				//printf("po is shotAl %d %d\n", po, point[po].shotAL);
				point[tmpdists[0].itsi].hitpoints -= 3; /*point[tmpdists[0].itsi].shotAL = 1;*/ if (point[tmpdists[0].itsi].hitpoints <= 0) {point[tmpdists[0].itsi].remove = 1; } 
			}
			return 0;
		}
		auto void attack(int mi);
		void attack(int mi) {
			if (point[mi].remove != 1 && point[po].remove != 1) {
				point[po].shotAL = 1;
