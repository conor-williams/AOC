...
int tX, tY, depth;
void next(int x, int y, int tool, int path, int (*)[], char (*)[]);
...
int main(int argc, char **argv)
{
	...
	int (*already)[tX+350] = malloc(sizeof(int[tY+351][tX+350]));
	...
	printit(0, 0, grid1);
	{
		...
		already1 = already;
		grid1 = grid;
		next(0,1, TOR, 1, already1, grid1);
		next(1,0, TOR, 1, already1, grid1);
...
int ind = 0;
void next(int x, int y, int tool, int path, int (*already1)[tX+350], char (*grid1)[tX+350]) {
	ind++;
	...
	if (path - 7 < already1[y][x]  || already1[y][x] == -1) {
		int newpath;
		switch(grid1[y][x]) { //10 neither //11 torch //12 climbing
			case WET:
				if (tool == TOR) {return;}
				if (path < already1[y][x]) {already1[y][x] = path;}
				for (int t = 10; t <= 12; t++) {
					if (t == TOR) {
						continue;
					} else {
						...
						{next(x, y-1, t, newpath, already1, grid1);}
						{next(x+1, y, t, newpath, already1, grid1);}
						{next(x, y+1, t, newpath, already1, grid1);}
						{next(x-1, y, t, newpath, already1, grid1);}
					}
				}
