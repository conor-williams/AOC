...
int traverse(string tra, string endPoint) {
	...
	for (int i = 0; i < iter->second.numNodes; i++) {
		for (int j = 0; j < ind; j++) {printf("	");} printf("****here%s\n", iter->second.to[i]); getchar();
	}
	for (int i = 0; i < iter->second.numNodes; i++) {
		...
		if ((ret = traverse(iter->second.to[i], endPoint)) == 33) {
			printf("ret (END) 33 TOT is %d split(%d)\n", tot, split[splitCounters]);
			tot -= split[splitCounters];
			printf("sub of  is %d\n", split[splitCounters]);
			split[splitCounters] = 0;
...
/*
//how many to the left of each
int traverseBack(string tra, string endPoint) {
	ind++;
	auto iter = Bmp.find(tra);
	...
	for (int i = 0; i < iter->second.numNodes; i++) {
		...
		if ((ret = traverse(iter->second.to[i], endPoint)) == 33) {
			...
			split++;
			ind--;
			printf("ret 0 TOT is %d split(%d)\n", tot, split);
			return 0;
		} else {
			split++;
			printf("ret %d TOT is %d split(%d)\n", ret, tot, split);
		}
	}	
	if (iter->second.numNodes == 0) {
		for (int j = 0; j< ind; j++) {printf("	");}
