	...
	for (auto iter2 = mp.begin(); iter2 != mp.end(); iter2++) 
	{
		...
		if (strcmp(iter2->first.c_str(), "FUEL") == 0) {
			for (int i = 0; i < iter2->second.numNodes; i++) {
				...
				search(iter2->second.from[i], (char *)"ORE", val, 0);
			}
		//	getchar();
			break;
		}
...
int search(char *start, char *end, double val, double valwas) {
	//for (auto iter2 = mp.begin(); iter2 != mp.end(); iter2++) 
	{
...
*/
					search(iter2->second.from[i], end, val*iter2->second.fromCost[i], valwas);
