...
void run(int);
unsigned long long runFOR(unsigned long long FRM);
...
int main(int argc, char **argv)
{
	...
	ENDLIST = ENDL;
	printf("argc is %d\n", argc);
	if (argc >= 3) {ENDLIST = strtoull(argv[2], &tmp, 10);}
	if (argc >= 4) {CARD = strtoull(argv[3], &tmp, 10);}
	lenx = 0; leny = 0;
...
fclose(a);
	...
	unsigned long long FRM4 = 2020;
	unsigned long long counter = 0;
	while (1) {
		unsigned long long ans4 = runBACK(FRM4);
		if (ans4 == 2020) {
			printf("2020 (BACK) again @ counter = %lld\n", counter); fflush(stdout); getchar();
		}
		FRM4 = ans4;
		counter++;
	}
	...
	unsigned long long FRM3 = 2020;
	for (long long i = 0; i < 2000000000000; i++) {
		if (i % 10000000 == 0) {printf("%lld\n", i);}
		unsigned long long ans3 = runFOR(FRM3);

		if (ans3 == 2020) {
			printf("2020 again @ i = %lld\n", i); fflush(stdout); getchar();
		}
		FRM3 = ans3;
	}
	printf("after...\n");
	...
	return 0;
	unsigned long long FRM2 = CARD;
	printf("ENDLIST is %llu FRM2 is %llu CARD %llu\n", ENDLIST, FRM2, CARD);
	for (int t = 0; t < 3; t++) {
		FRM2 = runBACK((unsigned long long)FRM2);
		printf("t:(%3d) ->  %16llu\n", t, FRM2);
	}
#else 
	unsigned long long ans4 = runFOR(2019);
	assert(ans4 == 2604);
	printf("is 2604...\n");
	deck.clear();
