...
void signal_handler(int x) {
	printf("signal_handler: -- myminPathLen:(%d) mylast3:[[%s]]\n",
		myminPathLen, mylast3);
}
...
void process(int ig, struct letters_s letters[], char upto[], int uptoPos, int mylen2);
int minPath;
...
cont:
	...
	int uptoPos = 0;
	int mylen = 0;
	process(-1, letters, upto, uptoPos, mylen);
...
int calcLen(char mynewup[27]) {
	int myPathLen;
	{
		myPathLen = mplen[{'@', mynewup[0]}];
		for (int j = 0; j < (int) strlen(mynewup)-1; j++) {
			myPathLen += mplen[{mynewup[j], mynewup[j+1]}];
		}
		return myPathLen;
	}
	return 0;
...
int ind = 0;
void process(int ig, struct letters_s letters[], char upto[], int uptoPos, int mylen1) {
	ind++;
...
again:
	...
	switch(uptoPos) {
		case 0:
			break;
		case 1:
			mylen1 += mplen[{'@', upto[uptoPos-1]}];
			if (mylen1 >= myminPathLen) {return;}
			break;
		default:
			mylen1 += mplen[{upto[uptoPos-2], upto[uptoPos-1]}];
			if (mylen1 >= myminPathLen) {return;}
			break;
	}
	...
	for (int i = 0; i < numlets; i++) {
		...
		string mine;
		if (upto[0] == '\0') {
			mine = mppat[{'@', letters[i].letkey}];
		} else {
			mine = mppat[{upto[uptoPos-1], letters[i].letlock}];
		}
		int found = 0;
		for (int j = 0;  j < (int)mine.length(); j++) {
			for (int k = 0; k < numlets; k++) {
				if (letters[k].ignore == 1) {continue;}

				if (mine[j] == letters[k].letlock) {
					found++;
				}
			}	
		}
	...
	if (numfound == 1) {
		...
		if (uptoPos == numlets) {
			strcpy(mylast3, upto);
			if (mylen1 < myminPathLen) {myminPathLen = mylen1; /*strcpy(minminP, upto); */}
		}
		ind--;
		return;
	} else if (numfound > 1) {
		...
		for (int z = 0; z < igsPos; z++) {
			...
			uptos[z][uptoPos+1] = '\0';
			process(igs[z], let2ormore[z], uptos[z], uptoPos+1, mylen1);
