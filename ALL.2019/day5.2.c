...
while(1) {
	...
	do {	
		...
		inputCounter = 0;
		if (count == 1) {/*INPUTCONOR*/input[0] = 5;}
	for (int i = 0; i < instTOT; i++) {
		...
		if (instruction[i][strlen(instruction[i])-1] == '3') {
			...
			if (instruction[i][3] == '2') {
				...
				if (val1 != 0) {printf("NONZ: JUMP to (%d)\n", val2); i = val2-1; err = 2;} else {printf("ZNOJUMP (%d)\n", val1); err = 1;}
			} else if (instruction[i][3] == '6') {
				if (val1 == 0) {printf("ZERO: JUMP to (%d)\n", val2); i = val2-1; err = 2;} else {printf("NONZ NOJMP (%d)\n", val1); err = 1;}
