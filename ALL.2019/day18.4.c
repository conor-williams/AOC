...
void process(int ig, struct letters_s letters[], char upto[], int uptoPos);
int minPath;
...
*/
	...
	char upto[30] = "";
	int uptoPos = 0;
	process(-1, letters, upto, uptoPos);
...
auto void getPos(int pos, int uplow, int which) {
	...
	for (int y = 0; y < leny; y++) {
		...
		printf("myPathLen is %d\n", myPathLen);
		if (myPathLen < myminPathLen) {myminPathLen = myPathLen;}
	}
	printf("** ans %d\n", myminPathLen);
	getchar();
...
again:
	...
	if (numfound == 1) {
		...
		for (int z = 0; z < igsPos; z++) {
			...
			uptos[z][uptoPos+1] = '\0';
			process(igs[z], let2ormore[z], uptos[z], uptoPos+1);
