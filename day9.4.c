...
void play(int players, int lastMarble) {
	...
	circle2.insert(circle2.end(), 0); //len3 //player2
	circle2.insert(circle2.end(), 4); //len3 //player2

	circle2.reserve(cSIZE);
	unsigned long long score[450] = {0};
	int curpos = 2;
	//rotate(circle2.begin(), circle2.begin()+1, circle2.end());
	curpos = 0;
	int origI = 4;
//	for (auto& k: circle2) { printf("%d ", k); } printf("\n"); getchar();
	for (int i = 5; i < cSIZE; i++) {
		...
		if ((origI % 23) != 0) {
			rotate(circle2.begin(), circle2.begin()+1, circle2.end());
			circle2.insert(circle2.end(), origI);
			//for (auto& k: circle2) { printf("%d ", k); } printf("\n"); getchar();
		} else {
	                score[pl] += origI;
			rotate(circle2.begin(), circle2.begin()+(i-7), circle2.end());
