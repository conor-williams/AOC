...
void buildit6(int *nums, int pos, int one, int two, int three, int four, int five, int six);
void buildit7(int *nums, int pos, int one, int two, int three, int four, int five, int six, int seven);
void buildit() ;
...
int main(int argc, char **argv)
{
	...
	while(1) {
		...
		printf("hashcode: |%s| len %d numdots %d\n", hashcode, 
				(int)strlen(hashcode), (int)strlen(hashcode) - tot2);
		//distribute ((int)strlen(hashcode) - tot2) over (pos+1)

		int distrib = (int)strlen(hashcode) - tot2;
		int mynumdots = pos+1;
		if (mynumdots == 7) {
...
int matchit (char *ans, char *hashcode)
{
	...
	if ( (int)strlen(ans) != (int)strlen(hashcode)) {printf("ERROR...."); getchar();}

	/*
	   matching |###.##.#....| with |?###????????|
	   (int)strlen ans v (int)strlen hashcode 12 v 12
hashcode: |?###????????| len 12
*/
