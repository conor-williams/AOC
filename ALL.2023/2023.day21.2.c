...
std::unordered_map<Point, std::size_t, hasher> create_count_map(const std::vector<std::string>& map, const Point& S) {
	...
	while(!queue.empty()) {
		...
		if ((int)current.row >= (int)row_size) {
			current.tile_row += 1;
			current.row -= row_size;
		} 
		if ((int)current.col < 0) {
			current.tile_col -= 1;
			current.col += col_size;
		} 
		if ((int)current.col >= (int)col_size) {
			current.tile_col += 1;
			current.col -= col_size;
		}
		// current.update_hash();
		if (current.row < 0 || (int)current.row >= (int)row_size || current.col < 0 || (int)current.col >= (int)col_size || map[current.row][current.col] == '#') continue;
...
int main(int argc, char * argv[]) {
	...
	for (int row = 0; row < (int)n_rows; row++) {
