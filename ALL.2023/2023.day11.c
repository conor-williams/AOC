...
int main(int argc, char **argv)
{
	...
	for (int y1 = 0; y1 < linenum; y1++) {
		for (int x1 = 0; x1 < (int)strlen(line[0]); x1++) {
			if (line[y1][x1] == '#') {
				...
				Ix2 = x1+1;
				Iy2 = y1;
				if (x1+1 == (int)strlen(line[0])) {Ix2 = 0; Iy2++;} 
				for (int y2 = Iy2; y2 < linenum; y2++) {
					for (int x2 = Ix2; x2 < (int)strlen(line[0]); x2++) {
						if (line[y2][x2] == '#') {
