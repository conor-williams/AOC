...
int main(int argc, char **argv)
{
	...
	printf("2023 Day 17 Part 1 alternative\n"); fflush(stdout);
				...
				if (pa[ii].first - dirX == 0 && pa[ii].second - dirY == 0) {
					if (ste <= 3) {
						...
						if (nx < 0 || nx > leny-1 || ny < 0 || ny > leny-1) {
							...
							pr.push({path+((int)grid[ny][nx]-48), nx, ny, dirX, dirY, ste+1});
						}
					}
					...
					//turn if he has gone at least 4 steps not more than 10
						int nx = x+ pa[ii].first;	
						int ny = y+ pa[ii].second;	
						if (nx < 0 || nx > leny-1 || ny < 0 || ny > leny-1) {
							continue;
						} else {
							pr.push({path+((int)grid[ny][nx]-48), nx, ny, pa[ii].first, pa[ii].second, 1});
