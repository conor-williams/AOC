...
struct xtra_s xtraBEG[1100][3000];
...
printf("end of first part..%d.\n", savesPos); 
	...
	for (int i = 0; i < savesPos; i++) {
		...
		for (int i = 0; i < 1050; i++) {savesBEGPos[i] = 0;}
		//////////////////////
...
void loop(int sv, int let, int first, int *pathLen, int theEnd, int sX, int sY) {
	...
	for (int r = 0; r < savesBEGPos[let]; r++) {
		...
		*pathLen = mypath;
		...
		if (ret == 0) {continue;} else if (ret == 1) {*pathLen = ret; check[r] = cpoint(let, r, *pathLen); loop(sv, let+1, 2, pathLen, (let+1)==(int)(strlen(saves[sv])-1), sX, sY);}
		if (ret == 2) {
...
//	if (gridTmp[prevy][prevx].avail >= gridTmp[myY][myX].used && (steps < already[myY][myX]  || already[myY][myX] == -1)) {
	...
	if (strlen(newpath) + 100 > 1100) {printf("ERR"); exit(0);}
