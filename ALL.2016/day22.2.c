...
void loop(int sv, int let, int first, int *pathLen, int theEnd, int sX, int sY, int myr, int myrTot) {
	...
	//int lens[2000];
	printf("savesBEGPos[let] is %d\n", savesBEGPos[let]);
	//for (int q = 0; q < savesBEGPos[let]; q++) { lens[q] = (int)savesBEG[let][q].size(); //(int)strlen(savesBEG[let][q]); }

	sort(savesBEG[let].begin(), savesBEG[let].end(), [](string a, string b) {return a.length() < b.length(); });
	//sort(savesBEG[let].end(), savesBEG[let].begin());

	int greater = check[0];
	//printf("begin()[%s](%d)  end() [%s](%d)\n", savesBEG[let][0].c_str(), 0, savesBEG[let][savesBEGPos[let]-1].c_str(), savesBEGPos[let]-1);
	printf("begin()[%s](%d)\n", savesBEG[let][0].c_str(), 0);
	int myend;
	if (savesBEGPos[let] > 4) {myend = 4;} else {myend = savesBEGPos[let];}
	for (int r = 0; r < myend; r++) {
		int newR = r;
		*pathLen = mypath;
		...
		if (ret == 0) {continue;} else if (ret == 1) {if (*pathLen < minPathLen 
#ifndef EX1
				&& *pathLen < 197
#endif
...
int carryitout(int myp, int letpos, int *pathLen1, int theEnd, int *sX, int *sY) {
	...
	int z = myp; 
	{
		...
		*sX = xtraBEG[letpos][z].sX; *sY = xtraBEG[letpos][z].sY;
		...
		int printit = 1;
		for (int q = 0; q < (int)savesBEG[letpos][z].size(); q++) {
			prevx = myX; prevy = myY;	
