...
char saves[1000][1000];
int savesPos = 0;
void savepathBEG(char path[], int x1, int y1, int letpos, int sX, int sY);
char savesBEG[1600][1000][1000];
int savesBEGPos[1600] = {0};
struct xtra_s {
	int x1;
	int y1;
	int x;
...
struct xtra_s xtraBEG[1600][3000];
...
int cpoint(int let, int r, int pathLen, int SAME) {
	...
	int usepos = -1;
	if (SAME == 1) {usepos = checkpointPos;}
...
/*
int cpoint(int let, int r, int pathLen) {
	if (checkpointPos >= 3500) {printf("ERROR cpoint\n"); exit(0);}
...
printf("end of first part..%d.\n", savesPos); 
	...
	//for (int i = 0; i < savesPos; i++) {/*37*/
	for (int i = 898; i < savesPos; i++) {/*37*/
	//for (int i = 897; i < 898; i++) {
		/*if (i % 10 == 0) */{printf("savesPos %d of %d (minPathLen %d)\n", i, savesPos, minPathLen);}
		for (int y5 = 0; y5 <= MAXY; y5++) { for (int x5 = 0; x5 <= MAXX; x5++) { grid[y5][x5] = gridOrig[y5][x5]; } }
		for (int y = 0; y <= MAXY; y++)    { for (int x = 0; x <= MAXX; x++)    { already[y][x] = -1;} }
		//printf("here %d  [%s]\n", i, saves[i]); getchar();

		for (int i = 0; i < 1200; i++) {savesBEGPos[i] = 0;}
		//////////////////////
...
void loop(int sv, int let, int first, int *pathLen, int theEnd, int sX, int sY) {
	...
	printf("calling generate %d\n", let); fflush(stdout);
	generateSAVESFORlettterPos(sv, let, sX, sY);
	printf("after generate %d\n", let); fflush(stdout);
	int mypath = *pathLen;
	int ret;
	...
	for (int r = 0; r < savesBEGPos[let]; r++) {
		for (int i = 0; i < ind; i++) {printf("	");} printf("r is %d\n", r);
		*pathLen = mypath;
		if (first == 1) {sX = MAXX; sY = 0; *pathLen = 0;}
		restore(check[r]-1, pathLen);
		...
		if (ret == 0) {continue;} else if (ret == 1) {/*pathLen = ret;*/ check[r] = cpoint(let, r, *pathLen, 0); loop(sv, let+1, 2, pathLen, (let+1)==(int)(strlen(saves[sv])-1), sX, sY);}
		if (ret == 2) {
...
void savepathBEG(char path[], int x1, int y1, int letpos, int x, int y, int sX, int sY, int frslotX, int frslotY, int toslotX, int toslotY) {
	...
	if (found == 0) {
if (savesBEGPos[letpos] >= 1600) {printf("ERROR too large...\n"); exit(0);}
...
//	if (gridTmp[prevy][prevx].avail >= gridTmp[myY][myX].used && (steps < already[myY][myX]  || already[myY][myX] == -1)) {
	...
	if (strlen(newpath)>= 1000) {printf("ERR"); exit(0);}
