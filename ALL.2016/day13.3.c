...
fclose(a);
	...
	int y = sY; int x = sX;
	if (y -1 < 0)    {} else { next(x, y-1); }
	if (x+1 >= GRID) {} else { next(x+1, y); }
	if (y+1 >= GRID) {} else { next(x, y+1); }
	if (x -1 < 0)    {} else { next(x-1, y); }
...
int next(int x, int y)
{
	if (grid[y][x] == '#') {
		return 0;
			...
			if (y -1 < 0)    {} else { if (already[y-1][x] != -1) {next(x, y-1);}}
			if (x+1 >= GRID) {} else { if (already[y][x+1] != -1) {next(x+1, y);}}
			if (y+1 >= GRID) {} else { if (already[y+1][x] != -1) {next(x, y+1);}}
			if (x -1 < 0)    {} else { if (already[y][x-1] != -1) {next(x-1, y);}}
