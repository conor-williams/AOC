...
void makemove(int myqs[], long steps, int first) {
	...
	for (int end = 1; end <= 2; end++) {
		...
		do {
			...
			if (first != 1) {
				if (qs[0] == 1 || qs[1] == 1) {} else {printf("skip1 %d steps: %ld\n", ind, steps); for (int i = 0; i < 4; i++) printf(" %d:%d ", i, qs[i]); printf("===\n"); printit(); continue;}
			...
			do {
				...
				if (tosort1[3] == 1 && tosort2[3] == 1 && tosort3[3] == 1 && tosort4[3] == 1) {
					...
					printf("**ind is %d\n", ind);
					printit();
					printf("END reached... %ld %ld\n", steps, minSteps); /*getchar();*/ ind--; return;
				} else {
					int found10 = 0;
					for (int j = 0; j < cpp; j++) {
						int found11 = 0;
